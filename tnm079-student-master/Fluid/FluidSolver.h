...
public:
  ...
  FluidSolver(float dx)
      : mDx(dx), mInitialVolume(0), mCurrentVolume(0), mExternalForces(NULL) {}
  ...
  void SetExternalForces(Function3D<Vector3<float>> *forces) {
    mExternalForces = forces;
  }
...
protected:
  ...
  inline void TransformGridToWorld(int i, int j, int k, float &x, float &y,
                                   float &z) {
    x = mBox.pMin[0] + i * mDx;
    y = mBox.pMin[1] + j * mDx;
    z = mBox.pMin[2] + k * mDx;
...
template <typename Real> Real norm(const std::vector<Real> &v) {
  return std::sqrt(dot(v, v));
}

template <typename Real>
std::vector<Real> operator-(const std::vector<Real> &v1,
