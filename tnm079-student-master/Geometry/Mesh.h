...
protected:
  ...
  virtual bool AddVertex(const Vector3<float> &v, size_t &indx) = 0;
  ...
  FindNeighborVertices(size_t vertexIndex) const = 0;
  ...
  virtual Vector3<float> FaceNormal(size_t faceIndex) const = 0;

  //! Compute and return the normal at vertex at vertexIndex
  virtual Vector3<float> VertexNormal(size_t vertexIndex) const = 0;
...
public:
  ...
  struct Face {
    Face(const Vector3<float> &n = Vector3<float>(0.0f, 0.0f, 0.0f),
         const Vector3<float> &c = Vector3<float>(0.5f, 0.1f, 0.7f),
         float u = 0)
  ...
  //! Minimal requirements for all meshes, inherited
  struct Vertex {
    Vertex(const Vector3<float> &p = Vector3<float>(0.0f, 0.0f, 0.0f),
           const Vector3<float> &n = Vector3<float>(0.0f, 0.0f, 0.0f),
           const Vector3<float> &c = Vector3<float>(0.5f, 0.1f, 0.7f),
           float u = 0)
