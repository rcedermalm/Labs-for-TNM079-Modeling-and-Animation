...
public:
  ...
  template <class MeshType> void Triangulate();
 ...
 */
template <class MeshType> void Implicit::Triangulate() {
  ...
  for (float i = pmin[0]; i < pmax[0] - 0.5 * mMeshSampling;
       i += mMeshSampling) {
    for (float j = pmin[1]; j < pmax[1] - 0.5 * mMeshSampling;
         j += mMeshSampling) {
      for (float k = pmin[2]; k < pmax[2] - 0.5 * mMeshSampling;
           k += mMeshSampling) {
        float voxelValues[8] = {
            GetValue(i, j, k),
        ...
        std::vector<Vector3<float>> tris =
            ::Triangulate(voxelValues, i, j, k, mMeshSampling);
        for (size_t n = 0; n < tris.size(); n += 3) {
          std::vector<Vector3<float>> verts;
          std::copy(tris.begin() + n, tris.begin() + n + 3,
