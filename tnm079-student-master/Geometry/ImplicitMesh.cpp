...
ImplicitMesh::ImplicitMesh(SimpleMesh *mesh) : mSourceMesh(mesh), mData(NULL) {
  ...
  Vector3<float> pad(0.1f, 0.1f, 0.1f);
  pMin -= pad;
  pMax += pad;
  mBox = Bbox(pMin, pMax);
...
void ImplicitMesh::Initialize() {
  ...
  mData = new Volume<float>(static_cast<int64_t>(ceil(dim[0])),
                            static_cast<int64_t>(ceil(dim[1])),
                            static_cast<int64_t>(ceil(dim[2])));
  ...
  int i, j, k;
  i = 0;
  for (float x = mBox.pMin[0]; x < mBox.pMax[0] - 0.5 * mMeshSampling;
       x += mMeshSampling, i++) {
    j = 0;
  ...
  i = 0;
  currentSample = 0;
  for (float x = mBox.pMin[0]; x < mBox.pMax[0] - 0.5f * mMeshSampling;
       x += mMeshSampling, i++) {
    j = 0;
...
float ImplicitMesh::DistanceToPoint(float x, float y, float z,
