...
public:
  ...
  //! Scale geometry
  void Scale(float s) { SetTransform(mTransform * Matrix4x4<float>::Scale(s)); }
  ...
  virtual void Dilate(float amount) {
    std::cout << "Dilate() not implemented for this type of geometry"
              << std::endl;
  }

  //! Erode geometry
  virtual void Erode(float amount) {
    std::cout << "Erode() not implemented for this type of geometry"
              << std::endl;
  }

  //! Smooth geometry
  virtual void Smooth(float amount) {
    std::cout << "Smooth() not implemented for this type of geometry"
              << std::endl;
  }
  ...
  //! Get transform
  Matrix4x4<float> &GetTransform() { return mTransform; }
  const Matrix4x4<float> &GetTransform() const { return mTransform; }
