...
//-----------------------------------------------------------------------------
bool SimpleMesh::AddVertex(const Vector3<float> &v, size_t &indx) {
  std::map<Vector3<float>, size_t>::iterator it = mUniqueVerts.find(v);
  if (it != mUniqueVerts.end()) {
    indx = (*it).second; // (*it).second == it->second, which to me was not as
                         // clear...
    return false;
  }

  mUniqueVerts[v] = indx =
      mVerts.size(); // op. [ ] constructs a new entry in map
  Vertex vert;
  vert.pos = v;
...
//-----------------------------------------------------------------------------
Vector3<float> SimpleMesh::FaceNormal(size_t faceIndex) const {
...
//-----------------------------------------------------------------------------
Vector3<float> SimpleMesh::VertexNormal(size_t vertexIndex) const {

  std::vector<size_t> neighborFaces = FindNeighborFaces(vertexIndex);

  Vector3<float> n(0, 0, 0);

  const size_t numCandidates = neighborFaces.size();
  ...
  if (mVisualizationMode == CurvatureVertex) {
    ...
    if (mAutoMinMax == false) {
      std::cerr << "Mapping color based on vertex curvature with range ["
                << mMinCMap << "," << mMaxCMap << "]" << std::endl;
      minCurvature = mMinCMap;
      maxCurvature = mMaxCMap;
    } else {
      std::cerr
          << "Automatic mapping of color based on vertex curvature with range ["
