...
struct Bbox {
  ...
  Bbox(const Vector3<float> &pmin = Vector3<float>(0, 0, 0),
       const Vector3<float> &pmax = Vector3<float>(0, 0, 0))
      : pMin(pmin), pMax(pmax) {}

  Bbox(float pmin, float pmax)
      : pMin(Vector3<float>(pmin, pmin, pmin)),
        pMax(Vector3<float>(pmax, pmax, pmax)) {}
  ...
  Bbox Transform(const Matrix4x4<float> &t) const {
    ...
    v = t * Vector4<float>(pMax[0], pMin[1], pMin[2], 1.f);
    b = PointUnion(b, v);
    v = t * Vector4<float>(pMax[0], pMax[1], pMin[2], 1.f);
    b = PointUnion(b, v);
    v = t * Vector4<float>(pMin[0], pMax[1], pMin[2], 1.f);
    b = PointUnion(b, v);

    v = t * Vector4<float>(pMin[0], pMin[1], pMax[2], 1.f);
    b = PointUnion(b, v);
    v = t * Vector4<float>(pMax[0], pMin[1], pMax[2], 1.f);
    b = PointUnion(b, v);
    v = t * Vector4<float>(pMax[0], pMax[1], pMax[2], 1.f);
    b = PointUnion(b, v);
    v = t * Vector4<float>(pMin[0], pMax[1], pMax[2], 1.f);
    b = PointUnion(b, v);
