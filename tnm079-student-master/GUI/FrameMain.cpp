...
FrameMain::FrameMain(wxWindow *parent) : BaseFrameMain(parent) {
  ...
  Connect(wxID_ANY, wxEVT_GL_OBJECT_SELECTED,
          wxCommandEventHandler(FrameMain::ObjectSelected));
...
// Add the panels to show for all corresponding types of GLObjects
  ...
  mPanelSwitches[typeid(UniformCubicSplineSubdivisionCurve).name()].push_back(
      mPanelTransform);
  mPanelSwitches[typeid(UniformCubicSplineSubdivisionCurve).name()].push_back(
      mPanelSubdivision);
  mPanelSwitches[typeid(LoopSubdivisionMesh).name()].push_back(mPanelTransform);
  mPanelSwitches[typeid(LoopSubdivisionMesh).name()].push_back(
      mPanelVisualization);
  mPanelSwitches[typeid(LoopSubdivisionMesh).name()].push_back(
      mPanelSubdivision);
#endif // Lab3
  ...
  mPanelSwitches[typeid(FluidVoxelCutPlane).name()].push_back(
      mPanelVisualization);
  mFluidSolver = NULL;
...
void FrameMain::RemoveObject(GLObject *object) {
  ...
  mObjectList->Delete(
      mObjectList->FindString(wxString(object->GetName().c_str(), wxConvUTF8)));
}
...
void FrameMain::ObjectSelected(wxCommandEvent &event) {
  ...
  while (iter != iend) {
    mObjectList->SetStringSelection(
        wxString((*iter)->GetName().c_str(), wxConvUTF8));
    iter++;
  }
...
void FrameMain::SelectObjects(wxCommandEvent &event) {
  ...
  for (int i = 0; i < numObjects; i++)
    mGLViewer->SelectObject(
        std::string(mObjectList->GetString(objects[i]).mb_str()));
}
...
void FrameMain::TransformObjects(wxCommandEvent &event) {
  ...
  if (!mScaleX->GetValue().ToDouble(&scaleX))
    scaleX = 1;
  if (!mScaleY->GetValue().ToDouble(&scaleY))
    scaleY = 1;
  if (!mScaleZ->GetValue().ToDouble(&scaleZ))
    scaleZ = 1;

  if (!mTranslateX->GetValue().ToDouble(&translateX))
    translateX = 0;
  if (!mTranslateY->GetValue().ToDouble(&translateY))
    translateY = 0;
  if (!mTranslateZ->GetValue().ToDouble(&translateZ))
    translateZ = 0;

  if (!mRotateX->GetValue().ToDouble(&rotateX))
    rotateX = 0;
  if (!mRotateY->GetValue().ToDouble(&rotateY))
    rotateY = 0;
  if (!mRotateZ->GetValue().ToDouble(&rotateZ))
    rotateZ = 0;
  ...
  while (iter != iend) {
    ...
    if (object == NULL) {
      ...
      object->Rotate(rotateX * M_PI / 180.0, rotateY * M_PI / 180.0,
                     rotateY * M_PI / 180.0);
...
void FrameMain::SetVisualizationMode(wxCommandEvent &event) {
  ...
  VisualizationModeData *mode = dynamic_cast<VisualizationModeData *>(
      mVisualizationModeChoice->GetClientObject(selection));
  while (iter != iend) {
  ...
  while (iter != iend) {
    ...
    if (mesh != NULL) {
      wxFileDialog *dialog = new wxFileDialog(
          this,
          _T("Save mesh '") + wxString(mesh->GetName().c_str(), wxConvUTF8) +
              _T("' as"),
...
mGLViewer->ScreenCapture(std::string(filename.mb_str()), mag);
  ...
  wxFileDialog *dialog =
      new wxFileDialog(this, _T("Save as"), _T("."), _T("dump.tga"),
...
void FrameMain::UpdatePanels() {
  ...
  while (iter != iend) {
    ...
    std::list<wxWindow *>::iterator panelIter =
        mPanelSwitches[(*iter)->GetTypeName()].begin();
    std::list<wxWindow *>::iterator panelIend =
        mPanelSwitches[(*iter)->GetTypeName()].end();
    ...
    std::list<GLObject::VisualizationMode> modes =
        (*iter)->GetVisualizationModes();
    ...
    while (modeIter != modeIend) {
      mVisualizationModeChoice->Append(
          wxString((*modeIter).GetName().c_str(), wxConvUTF8),
...
void FrameMain::DecimateObjects(wxCommandEvent &event) {
  ...
  while (iter != iend) {
    ...
    if (mesh == NULL)
      std::cerr << "Warning: Object '" << (*iter)->GetName()
                << "' is not a decimation mesh - can't be decimated"
                << std::endl;
    else {
      /*mesh->decimate();*/
      long targetFaces;
...
void FrameMain::AddObjectCubicSpline(wxCommandEvent &event) {
  ...
  c.push_back(Vector3<float>(-0.8f, 1.0f, 0.5f));
  c.push_back(Vector3<float>(-0.7f, 0.8f, 0.5f));
  c.push_back(Vector3<float>(-0.5f, 0.7f, 0.5f));
  c.push_back(Vector3<float>(-0.5, 1.0f, 0.5f));
  c.push_back(Vector3<float>(0.0f, 0.1f, 0.5f));
  c.push_back(Vector3<float>(0.3f, 0.6f, 0.5f));
  c.push_back(Vector3<float>(0.4f, 0.5f, 0.5f));
  c.push_back(Vector3<float>(0.5f, 1.0f, 0.5f));

  UniformCubicSpline *curve =
      new UniformCubicSpline(c, Vector3<float>(1.0f, 0.0f, 0.0f));
...
void FrameMain::AddObjectSubdivisionCurve(wxCommandEvent &event) {
  ...
  UniformCubicSplineSubdivisionCurve *curve =
      new UniformCubicSplineSubdivisionCurve(c);
...
void FrameMain::SubdivideObjects(wxCommandEvent &event) {
  ...
  while (iter != iend) {
    ...
    if (subdiv == NULL)
      std::cerr << "Warning: Object '" << (*iter)->GetName()
                << "' is not a subdivision object - can't be subdivided"
                << std::endl;
    else {
...
void FrameMain::AddObjectScalarCutPlane(wxCommandEvent &event) {
  ...
  while (iter != iend) {
    ...
    if (impl != NULL) {
      ...
      ScalarCutPlane *plane =
          new ScalarCutPlane("Scalar cut plane", 0.005f, field);
      AddUniqueObject(plane);
...
void FrameMain::AddObjectVectorCutPlane(wxCommandEvent &event) {
  ...
  while (iter != iend) {
    ...
    if (impl != NULL) {
      ...
      VectorCutPlane *plane =
          new VectorCutPlane("Vector cut plane", 0.1f, field);
      AddUniqueObject(plane);
...
void FrameMain::ConvertToLevelset(wxCommandEvent &event) {
  ...
  while (iter != iend) {
    ...
    if (impl != NULL) {
      ...
      Vector3<float> pad(0.2f, 0.2f, 0.2f);
      b.pMin -= pad;
      b.pMax += pad;
...
void FrameMain::AddTemplate1(wxCommandEvent &event) {
  ...
  // Setup bounding box
  Bbox b(Vector3<float>(-1.0f, -1.0f, -1.0f), Vector3<float>(1.0f, 1.0f, 1.0f));
...
void FrameMain::LevelsetMorph(wxCommandEvent &event) {
  ...
  if (LS == NULL) {
    std::cerr << "Error: Need to select at least one level set object"
              << std::endl;
    return;
  }
  ...
  while (iter != iend) {
    ...
    if (impl != NULL && impl != LS) {
      ...
      LS->SetBoundingBox(
          BoxUnion(LS->GetBoundingBox(), impl->GetBoundingBox()));
...
void FrameMain::InitializeFluidSolver() {
  if (mFluidSolver == NULL) {
    ...
    mFluidSolver->SetExternalForces(
        new ConstantVectorField(Vector3<float>(0.0f, -9.82f, 0.0f)));
  }
}
...
void FrameMain::FluidSolve(wxCommandEvent &event) {
  if (mFluidSolver == NULL) {
    std::cerr << "Error: Fluid solver not initialized - add solid or fluid"
              << std::endl;
    return;
  }
  ...
  for (long int i = 0; i < iterations; i++) {
    ...
    std::cerr << "Solving for fluid velocity field (time = " << time << ")..."
              << std::endl;
    mFluidSolver->Solve(time);

    // Update cut planes etc.
    std::list<std::string>::iterator iterObject =
        mFluidSolverDependentObjects.begin();
    std::list<std::string>::iterator iendObject =
        mFluidSolverDependentObjects.end();
    while (iterObject != iendObject) {
      Geometry *object =
          dynamic_cast<Geometry *>(mGLViewer->GetObject(*iterObject));
      if (object != NULL)
        object->Update();
      iterObject++;
    }
...
void FrameMain::FluidVisualizeVelocities(wxCommandEvent &event) {
  if (mFluidSolver != NULL) {
    VectorCutPlane *plane =
        new VectorCutPlane("Vector cut plane", 0.05f, mFluidSolver);
    AddUniqueObject(plane);
...
void FrameMain::FluidVisualizeVoxelsClassification(wxCommandEvent &event) {
  if (mFluidSolver != NULL) {
    FluidVoxelCutPlane *plane =
        new FluidVoxelCutPlane("Vector cut plane", mFluidSolver);
...
void FrameMain::SaveSimulationFrames(wxCommandEvent &event) {
  wxString filename;
  wxFileDialog *dialog = new wxFileDialog(
      this, _T("Save mesh sequence as"), _T("."), _T("fluid.obj"),
      _T("OBJ (*.obj)|*.obj"), wxFD_SAVE, wxDefaultPosition);
  if (dialog->ShowModal() == wxID_OK) {
