...
bool GLViewer::AddObject(GLObject *object) {
  ...
  while (iter != iend) {
    if ((*iter)->GetName() == object->GetName()) {
      std::cerr << "Error: GLObject '" << object->GetName()
                << " already added to scene" << std::endl;
      return false;
    }
    ++iter;
  }
  ...
  std::cerr << "Object '" << object->GetName() << "' added to scene"
            << std::endl;
...
void GLViewer::ReplaceObject(GLObject *oldObject, GLObject *newObject) {
  std::list<GLObject *>::iterator iter =
      std::find(mObjects.begin(), mObjects.end(), oldObject);
...
GLObject *GLViewer::PickObject(long x, long y) {
  ...
  gluPickMatrix(x, GetHeight() - y, 4.0, 4.0,
                viewport); // x, y, width, height is the picking area
  ...
  object->PickChildren(mSelectBuffer + pickedObject + 1,
                       mSelectBuffer[pickedObject - 3] - 1);
  return object;
}
...
void GLViewer::MouseMoved(wxMouseEvent &event) {
  ...
  if (event.LeftIsDown()) {
    if (event.AltDown()) {
      ...
      mCamera.QuatRotate((mMousePrevious[0] / halfWidth) - 1,
                         1 - (mMousePrevious[1] / halfHeight),
...
void GLViewer::MouseReleased(wxMouseEvent &event) {
  if (event.GetX() == mMouseDown[0] && event.GetY() == mMouseDown[1]) {
    std::cerr << "Pick at (" << event.GetX() << "," << event.GetY() << ")"
              << std::endl;
    ...
    if (object != NULL) {
      object->Select();
      std::cout << "Selecting object '" << object->GetName() << "'"
                << std::endl;
    }
