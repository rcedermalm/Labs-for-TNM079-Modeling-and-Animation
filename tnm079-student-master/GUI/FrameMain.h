...
protected:
  ...
  public:
    ...
    const GLObject::VisualizationMode &GetVisualizationMode() {
  ...
  template <class MeshType> MeshType *AddMesh(const wxString &path);
#endif // Lab1
...
#ifdef LAB1
template <class MeshType> MeshType *FrameMain::AddMesh(const wxString &path) {
    ...
    if (second != NULL) {
      ...
      if (mBlend->IsChecked()) {
        ...
        if (!mBlendParameter->GetValue().ToLong(&blend)) {
          std::cerr
              << "Error: can't parse CSG blend parameter - defaulting to 10"
              << std::endl;
