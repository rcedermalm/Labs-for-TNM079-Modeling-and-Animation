...
typename VectorT::ValueType Cotangent(const VectorT &a, const VectorT &b,
                                      const VectorT &c) {
 ...
 */
template <typename Scalar> inline Scalar Sign(Scalar value) {
...
inline size_t EndianSwap(size_t x) {
  return ((x >> 24) | ((x << 8) & 0x00FF0000) | ((x >> 8) & 0x0000FF00) |
          (x << 24));
}

//! Byte swap array, only works for 32bit types
template <typename T> inline void EndianSwap(T *data, size_t num) {
  if (sizeof(T) == sizeof(size_t)) {
    size_t *tmpPtr = reinterpret_cast<size_t *>(data);
    std::transform(tmpPtr, tmpPtr + num, tmpPtr,
                   std::pointer_to_unary_function<size_t, size_t>(&EndianSwap));
  } else {
    // badness
  }
}
