...
bool ObjIO::Load(Mesh *mesh, std::istream &is) {
  ...
  for (size_t t = 0; t < numTris; t++) {
    Vector3<size_t> &triangle = loadData.tris[t];
  ...
  while (!is.eof()) {
    ...
    case 'f': {
      ...
      if (count == 3) {
        ...
        std::cerr << "Encountered polygon with " << count
                  << " faces. Bailing out.\n";
        return false;
      }
    } break;
...
Vector3<size_t> ObjIO::ReadTri(std::istream &is) {
  ...
  return Vector3<size_t>(v) -
         Vector3<size_t>(1, 1, 1); // obj file format is 1-based
}
