...
bool DecimationMesh::decimate(size_t targetFaces) {
  ...
  std::cout << "Collapsed mesh to " << mFaces.size() - mNumCollapsedFaces
            << " faces" << std::endl;
...
bool DecimationMesh::decimate() {
  ...
  while (mEdges[mVerts[v2].edge].face == f1 ||
         mEdges[mVerts[v2].edge].face == f2)
  ...
  mHalfEdge2EdgeCollapse[mEdges[mEdges[e1].prev].pair] =
      mHalfEdge2EdgeCollapse[mEdges[e1].next];

  if (mHalfEdge2EdgeCollapse[mEdges[e2].next] != NULL)
    delete mHeap.remove(mHalfEdge2EdgeCollapse[mEdges[e2].next]);
  mHalfEdge2EdgeCollapse[mEdges[mEdges[e2].next].pair] =
      mHalfEdge2EdgeCollapse[mEdges[e2].prev];

  // Make sure the edge collapses point to valid edges
  if (mHalfEdge2EdgeCollapse[mEdges[e1].next] != NULL)
    mHalfEdge2EdgeCollapse[mEdges[e1].next]->halfEdge =
        mEdges[mEdges[e1].prev].pair;
  if (mHalfEdge2EdgeCollapse[mEdges[e2].prev] != NULL)
    mHalfEdge2EdgeCollapse[mEdges[e2].prev]->halfEdge =
        mEdges[mEdges[e2].next].pair;
  ...
  do {
    ...
    if (!isFaceCollapsed(face))
      updateFaceProperties(face);
    if (!isVertexCollapsed(vert))
      updateVertexProperties(vert);
...
void DecimationMesh::Render() {
  ...
  // Draw geometry
  const auto numTriangles = mFaces.size();
  for (size_t i = 0; i < numTriangles; i++) {
    ...
    /*
        ...
        // draw face
        sprintf(buffer, "f%i\n", i);
        Vector3<float> vec = (mVerts[edge->vert].pos +
  ...
  if (mVisualizationMode == CollapseCost) {
    ...
    while (iter != iend) {
      ...
      if (collapse != NULL) {
        if (minCost > collapse->cost)
          minCost = collapse->cost;
        if (maxCost < collapse->cost)
          maxCost = collapse->cost;
      }

      iter++;
    }
    std::cout << "Colormapping collapse cost with range: [" << minCost << ","
              << maxCost << "]" << std::endl;
    ...
    while (iter != iend) {
      ...
      if (!isEdgeCollapsed(ind)) {
        ...
        if (collapse == NULL)
          ...
          glColor3fv(
              mColorMap->Map(collapse->cost, minCost, maxCost).GetArrayPtr());
