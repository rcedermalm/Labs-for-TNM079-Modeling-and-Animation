...
public:
  ...
  Vector4(Real v0, Real v1, Real v2, Real v3) {
    v[0] = v0;
    v[1] = v1;
    v[2] = v2;
    v[3] = v3;
  ...
  //  Real operator[](size_t i) const { return v[i]; }
  ...
  Vector4 operator+(const Vector4 &vec4) const {
    return Vector4(v[0] + vec4.v[0], v[1] + vec4.v[1], v[2] + vec4.v[2],
                   v[3] + vec4.v[3]);
  }
  Vector4 operator-(const Vector4 &vec4) const {
    return Vector4(v[0] - vec4.v[0], v[1] - vec4.v[1], v[2] - vec4.v[2],
  ...
  Vector4 entryMult(const Vector4 &vec4) const {
    return Vector4(v[0] * vec4.v[0], v[1] * vec4.v[1], v[2] * vec4.v[2],
                   v[3] * vec4.v[3]);
  }
