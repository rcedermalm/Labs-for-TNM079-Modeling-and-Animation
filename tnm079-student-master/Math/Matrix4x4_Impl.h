...
const Matrix4x4<Real> Matrix4x4<Real>::identityMatrix = Matrix4x4<Real>();

template <typename Real> Matrix4x4<Real>::Matrix4x4() {
  m[0][0] = 1;
  m[0][1] = 0;
  m[0][2] = 0;
  m[0][3] = 0;
...
template <typename Real> Matrix4x4<Real>::Matrix4x4(const Real m[4][4]) {
  this->m[0][0] = m[0][0];
  this->m[0][1] = m[0][1];
  this->m[0][2] = m[0][2];
  this->m[0][3] = m[0][3];
...
template <typename Real> Matrix4x4<Real> Matrix4x4<Real>::Identity() {
...
Vector4<Real> Matrix4x4<Real>::operator*(const Vector4<Real> &vec4) const {
  ...
  res[0] = vec4[0] * m[0][0] + vec4[1] * m[0][1] + vec4[2] * m[0][2] +
           vec4[3] * m[0][3];
  res[1] = vec4[0] * m[1][0] + vec4[1] * m[1][1] + vec4[2] * m[1][2] +
           vec4[3] * m[1][3];
  res[2] = vec4[0] * m[2][0] + vec4[1] * m[2][1] + vec4[2] * m[2][2] +
           vec4[3] * m[2][3];
  res[3] = vec4[0] * m[3][0] + vec4[1] * m[3][1] + vec4[2] * m[3][2] +
           vec4[3] * m[3][3];
...
template <typename Real> Real &Matrix4x4<Real>::operator()(size_t i, size_t j) {
  assert(i >= 0 && i <= 3 && j >= 0 && j <= 3 &&
         "Index out of bounds in Matrix4x4");
  return m[i][j];
}

template <typename Real>
const Real &Matrix4x4<Real>::operator()(size_t i, size_t j) const {
  assert(i >= 0 && i <= 3 && j >= 0 && j <= 3 &&
         "Index out of bounds in Matrix4x4");
  return m[i][j];
}

template <typename Real> Real Matrix4x4<Real>::Determinant() const {
...
template <typename Real> Matrix4x4<Real> Matrix4x4<Real>::Transpose() const {
  Matrix4x4 A;
...
bool Matrix4x4<Real>::CholeskyFactorization(Matrix4x4 &U) {
  U = Matrix4x4();
  Matrix4x4 &A = *this;
  static const Real TOOSMALL((Real)0.0f);
...
template <typename Real> Matrix4x4<Real> Matrix4x4<Real>::Scale(Real scale) {
  Real m[4][4];
...
template <typename Real> Matrix4x4<Real> Matrix4x4<Real>::ToGLMatrix() const {
  Real glData[4][4];
