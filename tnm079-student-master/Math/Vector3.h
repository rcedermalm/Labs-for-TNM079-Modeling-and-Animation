...
v[2]=vec4[2]/vec4[3];
   ...
   */
  //! Copy constructor
  template <typename Real2> Vector3(const Vector3<Real2> &vec3) {
    *this = vec3;
  }
  //! Assignment operator = for vector
  template <typename Real2> Vector3 &operator=(const Vector3<Real2> &vec3) {
    v[0] = (Real)vec3[0];
    v[1] = (Real)vec3[1];
    v[2] = (Real)vec3[2];
    return *this;
  ...
  template <typename Scalar> Vector3<Real> operator*(Scalar s) const {
    return Vector3<Real>((Real)(s * v[0]), (Real)(s * v[1]), (Real)(s * v[2]));
  }
  template <typename Scalar> Vector3<Real> &operator*=(Scalar s) {
    v[0] *= (Real)s;
    v[1] *= (Real)s;
    v[2] *= (Real)s;
    return *this;
  ...
  //! Templated equal operator for lexicographic comparison of vectors of
  //! different type
  template <typename Real2> bool operator==(const Vector3<Real2> &vec3) const {
    return v[0] == (Real)vec3[0] && v[1] == (Real)vec3[1] &&
           v[2] == (Real)vec3[2];
  }

  // Geometric operators
  //! Norm of a vector
  Real Norm() const { return v[0] * v[0] + v[1] * v[1] + v[2] * v[2]; }
...
protected:
  ...
  friend Vector3<Real2> Cross(const Vector3<Real2> &v1,
                              const Vector3<Real2> &v2);
};

//! Specialization of length for float type, uses sqrtf instead.
template <> inline float Vector3<float>::Length() const {
...
template <typename Real> istream &operator>>(istream &is, Vector3<Real> &m) {
  m.Load(is);
  return is;
}
} // namespace std
