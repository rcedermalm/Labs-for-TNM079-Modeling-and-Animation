 ...
 */
template <class Matrix, class Vector, typename Real> class ConjugateGradient {
  size_t mMaxIter, mIter;
  Real mMaxTolerance, mTolerance;

public:
  ConjugateGradient(size_t maxiter, Real tolerance)
      : mMaxIter(maxiter), mIter(0), mMaxTolerance(tolerance), mTolerance(-1) {}
  size_t getNumIter() const { return mIter; }
  size_t getMaxNumIter() const { return mMaxIter; }
  Real getTolerance() const { return mTolerance; }
  Real getMaxTolerance() const { return mMaxTolerance; }
  /*! The actual algorithm, returns a boolean indicating whether the method
   * converged to the given threshold within the maximum allowed iterations. Use
   * get/set methods above to query statistics.
   */
  bool solve(Matrix &A, Vector &x, const Vector &b) {
    Real resid;
    Vector p, q;
    Real alpha, beta, rho, rho_1;
    size_t max_iter = mMaxIter;
