...
public:
  ...
  Volume(int dimX, int dimY, int dimZ)
      : mData(dimX * dimY * dimZ, T()), mDimX(dimX), mDimY(dimY), mDimZ(dimZ),
        mPremult(dimY * dimZ) {}

  Volume(int dimX, int dimY, int dimZ, T defaultVal)
      : mData(dimX * dimY * dimZ, defaultVal), mDimX(dimX), mDimY(dimY),
        mDimZ(dimZ), mPremult(dimY * dimZ) {}
  ...
  size_t ComputeLinearIndex(int i, int j, int k) const {
    i = Clamp(i, 0, mDimX - 1);
    j = Clamp(j, 0, mDimY - 1);
    k = Clamp(k, 0, mDimZ - 1);
  ...
  void Load(std::istream &is) {
    ...
    std::cerr << "Loading dims: " << mDimX << ", " << mDimY << ", " << mDimZ
              << std::endl;
  ...
  void Load(const std::string &path) {
    ...
    if ((file = fopen(path.c_str(), "rb")) != NULL) {
      ...
      if (w == 4) {
        ...
        fclose(file);
        std::cerr << "Error: Only float volumes supported, file bit depth = "
                  << w << std::endl;
        return;
      }
